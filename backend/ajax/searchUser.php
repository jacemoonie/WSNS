...
if(is_post_request()){
    if(isset($_POST['searchUser']) && !empty($_POST['searchUser'])){
        ...
        $search = FormSanitizer::formSanitizerString($_POST['searchUser']);
        ...
        if(!empty($result)){
            foreach($result as $user){
                if($user->user_id != $userid){
                    ...
                    $isFriend = $loadFromFriend->is_already_friends($userid, $user->user_id);
                    $alreadySendRequest = $loadFromFriend->am_i_the_req_sender($userid, $user->user_id);
                    if(!$isFriend && !$alreadySendRequest){
                        echo '
                        <li role="option" aria-selectable="true" >
                        <div role="button" tabindex="0" data-focusable="true" id="add-user" class=" h-ment" data-profileId="'.$user->user_id.'">
                            <div class="ment-w-container">
                                <div class="ment-profile-wrapper">
                                    ...
                                    <div class="col-3 add-participant-btn" > 
                                        <a href="'.url_for('friend').'?action=send_req&id='.$user->user_id.''.'" class="req_actionBtn Button sendRequest" id="add-friend" data-uid="'.$user->user_id.'">Send Request</a>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    if(isset($_POST['editParticipant']) && !empty($_POST['editParticipant'])){
        $userid = h($_POST['userId']);
        $search = FormSanitizer::formSanitizerString($_POST['editParticipant']);
                    ...
                    <div role="button" tabindex="0" data-focusable="true" id="add-user" class=" h-ment" data-profileId="'.$user->user_id.'">
                        <div class="ment-w-container">
                            ...
                            <div class="ment-profile-wrapper">
                                ...
                                <div class="col-3 add-participant-btn" > 
                                    <button id="edit-add-participant" data-uid="'.$user->user_id.'" class="Button">Add</button>
                                </div>
                            </div>
                        </div>
                    </div>
