...
class Friend{
    ...
    public function request_notification($my_id, $send_data){
        try{
            $sql = "SELECT sender, username, profileImage FROM `friend_request` JOIN users ON friend_request.sender = users.user_id WHERE receiver = ?";
                ...
                if($send_data){
                    ...
                    foreach($all_users as $row){
                        if($row->user_one == $my_id){
                            $get_user = "SELECT `user_id`, username, profileImage FROM `users` WHERE `user_id` = ?";
